...
response=requests.get("https://tgftp.nws.noaa.gov/data/observations/metar/cycles/01Z.TXT")
...
d = {"station":station, "lat":lat,"lon":lon,"time":time,"weather":weather, "weatherfont":weatherfont,"weathersymbol":weathersymbol,"temperature":temperature,"pressure":pressure,"sl_pressure":sl_pressure,"dewpoint":dewpoint, "windspeed":windspeed, "winddir":winddir}
df=pd.DataFrame(d)
#metardata = df.loc[:, ~(df == '99999').any()]
print(df.tail())

metar_csv_data = df.to_csv('metardata.csv', index = True)
