...
from ressources.tools.imuktools import metarww, pressurereduction
...
response=requests.get("https://aviationweather.gov/adds/dataserver_current/httpparam?dataSource=metars&requestType=retrieve&format=xml&minLat=25&minLon=-50&maxLat=72&maxLon=39&startTime=2023-03-15T23:59:00Z&endTime=2023-03-16T01:00:00Z")#+timedif+"Z.TXT")
data = pd.read_csv('../../Data/airport_europe.csv') #Dataframe with all Stations of europe
...
#metar_csv_data = df.to_csv('BUFR/metardata.csv', index = True)
metarcompare = duplicatedremove.to_csv('metarcompares.csv', index = True)
